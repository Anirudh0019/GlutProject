// 2nd batch changes - some function prototype for drawCowboy, its function , commented out "drawAlienBody,drawAlienFace,drawAlienEyes"
// 3rd batch changes -  commented some part inside SpaceshipCreate(), modified cowboy to change gun
//issue 1- DrawSpaceshipBody() messes up the speed
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <gl/glut.h>
...
// 2nd batch changes here (only line below)
void drawCowboy(float x, float y, float angle);
...
void SetDisplayMode(int modeToDisplay) {
	...
	case GAME_SCREEN: glClearColor(1, 1, 0.8, 1); break;
...
//	DrawAlienEyes();
//}
// i dont see the issue here this is messed up
...
void drawCowboy(float x, float y, float angle) {
	...
	glRotatef(angle, 0.0f, 0.0f, 1.0f); // Rotate the cowboy based on the angle

	// Draw the cowboy body
	glColor3f(1.0f, 0.5f, 0.0f); // Brown color for the body
	glBegin(GL_POLYGON);
	...
	// Draw the gun
	glColor3f(0.5f, 0.5f, 0.5f); // Gray color for the gun
	glBegin(GL_POLYGON);
...
//3 --> commendted some parts will denotes with !!! , ! for every batch no.
//2--> change here below // first change was that line , 2nd change will be the cowboy
void SpaceshipCreate() {
	glPushMatrix();
	glTranslated(xOne, yOne, 0);
	if (!checkIfSpaceShipIsSafe() && alienLife) {
		alienLife -= 10;
		if (alienLife < 0)alienLife = 0;
		xHealthBarStart -= 240; //changes
	}
	// changes above alienlife <0 line 
	//DrawSpaceshipDoom();//!
	glPushMatrix();
	glTranslated(4, 19, 0);
	drawCowboy(0.0f, 0.0f, LaserAngle); // DrawAlien existed here without any parameters
	glPopMatrix();
	//DrawSteeringWheel();//!
	DrawSpaceshipBody();//! -------- this is the function that messes up the speed , lets fix this
	DrawSpaceShipLazer();
	if (mButtonPressed) {
		DrawLazerBeam();
	}
...
// !-->>>>>>>>>>>>>>>>>>>>>>>>>>> 1st change this function now below line 350
...
//	//glColor3f(1, 0, 0);
//}
// iterations from here onwards everything that prev existed will be shown by 1--> etc
...
void DisplayHealthBar() {
	...
	float startX = -1150.0f; // Starting position for hearts
	float startY = 650.0f; // Y position for hearts
	float heartSize = 30.0f; // Size of each heart
	float spacing = 50.0f; // Spacing between hearts

	for (int i = 0; i < alienLife / 10; i++) {
		drawHeart(startX + i * spacing, startY, heartSize);
	}

	// Display score and level
	char temp[40];
	glColor3f(153.0f / 255.0f, 204.0f / 255.0f, 1.0f);
	sprintf_s(temp, "SCORE = %d", Score);
	displayRasterText(-1100, 600, 0.4, temp);
	sprintf_s(temp, "  LEVEL : %d", int(Score / 10));
	displayRasterText(-100, 600, 0.4, temp);
}
// the function for the existence of a cowboy instead of alien 
...
// the function below gets replaced ( 2nd batch of changes)
...
//}
void somethingMovedRecalculateLaserAngle() {
	float dx = mouseX - xOne;
	float dy = mouseY - yOne;
	LaserAngle = atan2(dy, dx) * 180.0f / PI; // Calculate the angle in degrees
}
